...
/* ---------------- season stats ---------------- */
const meRow = D.standings.find(s => s.entry === ME);
const leader = D.standings[0];
const myWeeks = GWS.map(gw => D.season[gw].find(r => r.entry === ME));
const myGwRows = GWS.map(gw => D.gameweeks[gw].rows.find(r => r.entry === ME));
const best = myWeeks.reduce((a, b) => (b.points > a.points ? b : a));
const worst = myWeeks.reduce((a, b) => (b.points < a.points ? b : a));
const bestGw = GWS[myWeeks.indexOf(best)], worstGw = GWS[myWeeks.indexOf(worst)];
const benched = myGwRows.reduce((s, r) => s + r.bench, 0);
const hits = myGwRows.reduce((s, r) => s + r.hit, 0);
const wins = GWS.filter(gw => D.gameweeks[gw].rows[0].entry === ME).length;
const avgPlace = myGwRows.reduce((s, r) => s + r.place, 0) / myGwRows.length;
const gap = leader.total - meRow.total;

$("standing").innerHTML = `
  <div class="pos num">${meRow.rank}<sup>${ord(meRow.rank).slice(String(meRow.rank).length)}</sup></div>
  <div class="who">
    <h2>${esc(meRow.team)}</h2>
    <p>${HAS_NAMES ? esc(meRow.manager) + " &middot; " : ""}${meRow.total} points &middot;
       ${gap === 0 ? "leading the league" : `${gap} behind ${esc(leader.team)}`}</p>
  </div>
  <div class="verdict">
    Best week <b>GW${bestGw} (${best.points})</b>, worst <b>GW${worstGw} (${worst.points})</b>.
    You have left <b>${benched} points on the bench</b> and paid <b>${hits} in hits</b>.
  </div>`;

$("stats").innerHTML = [
  { k: "Position", v: meRow.rank, n: `of ${D.managers.length}` },
  { k: "Total points", v: meRow.total, n: `${(meRow.total / GWS.length).toFixed(1)} per week` },
  { k: "Gap to top", v: gap === 0 ? "-" : "&minus;" + gap, n: gap === 0 ? "you lead" : esc(leader.team), cls: gap === 0 ? "good" : "bad" },
  { k: "Best gameweek", v: best.points, n: `gameweek ${bestGw}` },
  { k: "Left on bench", v: benched, n: "never counted" },
  { k: "Gameweeks won", v: wins, n: `avg finish ${avgPlace.toFixed(1)}` },
].map(t => `<div class="stat"><div class="k">${t.k}</div>
  <div class="v num ${t.cls || ""}">${t.v}</div><div class="n">${t.n}</div></div>`).join("");
...
function renderDiff() {
  ...
  const myDiff = mine.starters.filter(p => own[p] === 1).sort((a, b) => (pts[b] || 0) - (pts[a] || 0));
  const missed = Object.keys(own).filter(p => own[p] === 1 && !started.has(+p))
    .sort((a, b) => (pts[b] || 0) - (pts[a] || 0)).slice(0, 7);
  ...
  $("diff-caption").innerHTML =
    `Gameweek ${activeGw}. Yours returned <b>${gained}</b>; the best you missed returned <b>${lost}</b>.`;
  $("diff-mine").innerHTML = myDiff.length
    ? `<ul class="dlist">${myDiff.map(item).join("")}</ul>`
    : `<p class="empty">Nothing - your XI was the league template.</p>`;
